...
use train_lib::{artifact::{instance::{Instance, InstanceNumbers, InstanceStatus}, dao::ArtifactDao, dao::InstanceDao, Artifact, ArtifactStatus}, error::{self, GeneralError}, *};
...
fn process(queue: &queue::Queue) -> error::Result<Vec<String>> {
        // Block on reading the head of the list.
    let mut conn = artifact::dao::connection(DEFAULT_REDIS_URL).expect(&format!("Failed to establish connection to redis server at: {}", DEFAULT_REDIS_URL));
...
mod tests {
    ...
    fn test_process() {
        ...
        let mut artifact_request: artifact::ArtifactRequest = serde_json::from_str(request).expect("failed to deserialize the rollout request");
        ...
        for inst in instances {
            let _result = artifact::pipeline::delete_run(&inst.run_name, "train");
